    ...
    <>
      <section className="section-pad">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeading eyebrow="Our Story" title="Empowering Life with Solar, One Rooftop at a Time" subtitle="MRS Powertech is a Chennai-based solar energy company delivering premium residential, commercial and industrial solar solutions — from consultation and installation to government subsidy assistance and long-term maintenance." align="left" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-16">
            {[
              ...
              <motion.div key={v.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="card-luxury p-7">
          ...
          <SectionHeading eyebrow="Our Journey" title="Milestones" align="left" />
          <div className="relative border-l border-gold/20 pl-8 space-y-10 mb-20">
            {TIMELINE.map((t, i) => (
              <motion.div key={t.year} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="relative">
                <div className="absolute -left-[2.55rem] top-1 w-4 h-4 rounded-full bg-gold-gradient" />
                <span className="text-gold font-display font-bold text-lg">{t.year}</span>
                <h3 className="font-semibold text-lg mt-1">{t.title}</h3>
                <p className="text-white/55 text-sm mt-1 max-w-xl">{t.desc}</p>
              </motion.div>
            ))}
          </div>

          <SectionHeading eyebrow="Our People" title="Meet Our Team" align="left" />
